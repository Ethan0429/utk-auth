...
async def send_async_response(interaction, full_name, assignment):
    ...
    if assignment is None:
        ...
        bot_utils.md_to_png(
            f'{data["student_name"]}.png',
            canvas_utils.return_assignment_info(data, found_assignment),
        )
...
@bot.tree.command(
    name="auth",
    description="Sends authentication email",
    guild=discord.Object(id=bot_vars.GUILD_ID),
)
async def auth(interaction: discord.Interaction, netid: str):
    ...
    if bot_utils.is_auth(member):
        await interaction.response.send_message(
            f"{member.mention} You're already authenticated. Nothing to see here!",
            ephemeral=True,
        )
        return
    if netid is None:
        print("netid invalid!")
        await interaction.response.send_message(
            f"{member.mention} Please enter a valid UTK NetID!", ephemeral=True
        )
        return
    if bot_utils.key_exists(id):
        print(f"{netid} already has an authid!")
        await interaction.response.send_message(
            f"{member.mention} there already exists a passcode for this NetID. Type `/reset` to reset your passcode.",
            ephemeral=True,
        )
        return
    ...
    embed = (
        discord.Embed(
            title=f"Verification code sent!",
            description=f"{member.mention} Check your UTK email's **spam** inbox and follow the instructions written.",
            color=0xFFCC00,
            type="rich",
        )
        .add_field(name="Gmail", value=gmail_response, inline=False)
        .add_field(name="Outlook", value=outlook_response, inline=False)
    )
...
@bot.tree.command(
    name="verify",
    description="Verifies pin from auth email",
    guild=discord.Object(id=bot_vars.GUILD_ID),
)
async def verify(interaction: discord.Interaction, passkey: str):
    ...
    if passkey == auth_id["passkey"]:
        ...
        await interaction.response.send_message(
            f"{member.mention} Authentication successful!", ephemeral=True
        )
...
@bot.tree.command(
    name="reset",
    description="Resets verification pin",
    guild=discord.Object(id=bot_vars.GUILD_ID),
)
async def reset(interaction: discord.Interaction):
    ...
    if bot_utils.is_auth(member):
        await interaction.response.send_message(
            f"{member.mention} you're already authorized!", ephemeral=True
        )
        return
    if await bot_utils.get_auth_id(id) == None:
        await interaction.response.send_message(
            f"{member.mention} there is no code on file for your discord account!\nRun `/auth [netid]` (without brackets) command for starting the authentication process",
            ephemeral=True,
        )
        return
    bot_utils.remove_member(id)
    await interaction.response.send_message(
        f"{member.mention} your passcode has been reset!", ephemeral=True
    )
    return
...
async def kick_old(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        f"{interaction.user.mention} The following users will be kicked: {users_string}\nAre you sure you want to kick these users? (y/n)",
        ephemeral=True,
    )
    ...
    if response.content.lower() != "y":
        await interaction.channel.send(
            content=f"{interaction.user.mention} Kicking cancelled!", delete_after=5
        )
        return
...
@bot.tree.command(
    name="canvas",
    description="Gets Canvas info for a user.",
    guild=discord.Object(id=bot_vars.GUILD_ID),
)
@app_commands.autocomplete(full_names=full_names_autocomplete)
async def canvas(
    interaction: discord.Interaction, full_names: str, assignment: str = None
):
    ...
    await interaction.response.send_message(
        f"Processing your request, {interaction.user.mention}. Please wait...",
        ephemeral=True,
    )

    # Create a separate task to handle the API request and send the response when ready
    asyncio.create_task(send_async_response(interaction, full_names, assignment))
