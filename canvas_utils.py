...
def get_student_names():
    ...
    try:
        ...
        nicknames = {
            catch_invalid_login_id(user): catch_invalid_login_name(user)
...
class StudentDashboard:
    ...
    def get_user(self, username):
        return self.course.get_users(
            enrollment_type=["student"], include=["enrollments"], search_term=username
        )
    ...
    def get_section(self, user_id):
        sections = self.course.get_sections(include=["students", "enrollments"])
    ...
    async def get_student_info(self, username):
        ...
        submissions = [
            self.get_submissions(assignment)[0] for assignment in assignments
        ]
        ...
        for assignment, submission in zip(assignments, submissions):
            ...
            assignment_data["submission"][
                "url"
            ] = f"https://utk.instructure.com/courses/{bot_vars.CANVAS_COURSE_URL_ID}/assignments/{assignment_data['id']}"
            assignment_data["submission"][
                "submission_url"
            ] = f"https://utk.instructure.com/courses/{bot_vars.CANVAS_COURSE_URL_ID}/assignments/{assignment_data['id']}/submissions/{data['student_id']}"
...
def save_student_info_to_md(student_info, output_dir="output"):
    ...
    with open(file_path, "w") as md_file:
        md_file.write("# Student Information\n")
        md_file.write("\n")
        md_file.write(f"**Student ID**: {student_info['student_id']}\n")
        md_file.write(f"**Student Name**: {student_info['student_name']}\n")
        md_file.write("\n")
        md_file.write(f"**Student NetID**: {student_info['student_login_id']}\n")
        md_file.write("\n")
        md_file.write("## Course\n")
        md_file.write("\n")
        ...
        for assignment in student_info["assignments"]:
            ...
            md_file.write("\n")
            md_file.write("**Description**: <snip>\n")
            md_file.write("\n")
            md_file.write(f"**Due At**: {assignment['due_at']}\n")
            md_file.write("\n")
            md_file.write(f"**Attempt**: {assignment['submission']['attempt']}\n")
            ...
            md_file.write(f"**Missing**: {assignment['submission']['missing']}\n")
            md_file.write(
